...
We can again generate a krona plot for the results.
Which environment do we need?

```{r, eval=F, class.source='klippy'}
ktImportTaxonomy -t 5 -m 3 -o ~/Results/biobakery/example.metaphlan.html ~/Results/biobakery/example.metaphlan
```
...
```{r, eval=F, class.source='klippy'}
# samtools sort -o ~/Results/assembly/example/example.remap.sorted.bam ~/Results/assembly/example/example.remap.bam
# featureCounts -O -t CDS -g ID -a ~/Results/assembly/example/ -o ~/Results/assembly/example/example.featurecount ~/Results/assembly/example/example.remap.sorted.bam -p
```
...
Various microbes regulate there energy metabolism through carbon control protein A (CCPA). Mutation in this gene result in specific alterations of the fermentation profiles. Thus identificantion of these mutans in metagenome give 
